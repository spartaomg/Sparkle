Friend Module ModLZ
    ...
    'Private LitBuffer(255) As Byte
    'Private MatchBuffer(255) As Byte
            ...
            Else
                ...
                Else
                    ...
                    If (MaxOffset <= 64) And DataFits(1, 1, LitCnt, CheckIO(POffset - MaxLen + 1)) = True Then
                        ...
                        'ElseIf DataFits(1, 2, 0) = True Then

                    Else
                        'Check it 1 byte would fit as a literal...
                        GoTo CheckLit
...
CheckLit:
                'Match does not fit, check if we can add byte as literal
                Select Case LitCnt + 1
                    Case 0          'In case we jump here from below...
                        Bits = 2
                    Case 1 To 4
                        Bits = 3 + 2
                    Case 5 To 12
...
OML:                    'OverMax:
                    For I As Integer = POffset + 2 To MatchStart - 1
                        If (Prg(POffset + 1) = Prg(I)) And (Prg(POffset + 2) = Prg(I + 1)) And (I < POffset + 1 + 256) Then
                            ...
                            MaxLen = 1
                            'Debug.Print("Part: " + PartCnt.ToString + vbTab + "Block: " + (BlockCnt + 1).ToString + vbTab + (LitCnt + MaxLit + 1).ToString)
                            LitCnt = LitCnt + MaxLit + 1 - 2 '= 0/1 + 44 + 1 - 2 = 43/44
                            ByteCnt += 2             'Step back 2 bytes
                            POffset += MaxLen + 1   'Repositon POffset

                            Buffer(BitCnt) = 0       'Delete unused bit stream data (deleting 8+1 bits)
                            BitCnt -= 1              'Step back 8+1 bits
                            ...
                            If BitPos > 15 Then     'So we are now removing 8+1 bits from the bit stream, and also update the BitPos pointer
                                BitPos -= 8
                                Buffer(BitCnt) = 0   'Delete unused bit steam data
                                BitCnt -= 1
                            End If
                            Buffer(BitCnt) = Buffer(BitCnt) And (&H100 - (2 ^ (BitPos - 7)))  'Delete bit stream
                            AddLitBits()            'We are re-adding bits for 43/44 Literal bytes
                ...
                'GoTo UnderMax
                'End If
            'Case 0 + 1, 0 + 2, 4 + 1, 4 + 2, 12 + 1, 12 + 2                         'rangemax+1,rangemax+2 (value cannot be 0!!!) (rangemax: 0,4,12,44)
            Case 0 + 1, 0 + 2, 4 + 1, 4 + 2, 12 + 1, 12 + 2                         'rangemax+1,rangemax+2 (value cannot be 0!!!) (rangemax: 0,4,12,44)
                If (LitCnt = 1) And (OverMaxLit = True) Then
                    GoTo OML
                End If
                'UnderMax:
                For I As Integer = POffset + 2 To MatchStart - 1
...
Err:
    ...
    Private Sub UpdatePreviousMatch(PO As Integer)
...
Mid2Short:
                            Buffer(LastByteCt) = (J - 1) * 4 + LastMLen
                            ByteCnt = LastByteCt - 1
                            Exit For
                        End If
                    Case 2  'MatchLen=3
    ...
    Private Sub AddLitBits()
        ...
        If FirstOfNext = False Then AddRBits(1, 1)               'Add Literal Selector if this is not the first (Literal) byte in the buffer
...
Err:
    ...
    Private Sub AddRBits(Bit As Integer, BCnt As Byte)
        ...
        Bits = Buffer(BitCnt) * 256  'Load last bitstream byte to upper byte of Bits
        ...
        Buffer(BitCnt) = Int(Bits / 256) 'Save upper byte to buffer

        If BitPos < 8 Then  'Lower byte is affected, save it to buffer, too
            BitCnt += 1      'Next byte in BitStream
            Buffer(BitCnt) = Bits Mod 256
...
Err:
    ...
    Private Sub CloseBuff() 'CHANGE TO PUBLIC
        On Error GoTo Err
        ...
        If (MatchStart - POffset <= LastByte) And (MatchStart > LastByte - 1) And (NextFile = False) Then
            ...
            'If ByteCt - 1 > BitCt Then
            'If POffset > -1 Then
            'Buffer(ByteCt - 1) = Prg(POffset)
            'POffset -= 1
            'End If
            'End If
...
Err:
    ...
    Private Sub CloseLastBuff() 'CHANGE TO PUBLIC
        On Error GoTo Err
...
Err:
    ...
    Private Sub ResetBuffer() 'CHANGE TO PUBLIC
        On Error GoTo Err
