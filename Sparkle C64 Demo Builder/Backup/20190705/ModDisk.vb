        ...
        If FileCnt > -1 Then
            For FC = 0 To FileCnt
                If DFDiskNoA(FC) = DiskIndex Then   'Is this file on the current disk?
                    ...
                    End If
                    'Add file to part
                    LZ(FileNameA(FC), FileAddrA(FC), FileOffsA(FC), FileLenA(FC))
...
NoDisk:
    ...
    Private Function FinishDisk(LastDisk As Boolean) As Boolean
        ...
        If AddPartToDisk() = False Then GoTo NoDisk
        If InjectLoader(-1, 18, 5, 6) = False Then GoTo NoDisk
        If InjectDriveCode(DiskCnt + 1, PartCnt, IIf(LastDisk = False, DiskCnt + 2, 0)) = False Then GoTo NoDisk


        BytesSaved += Int(BitsSaved / 8)
        BitsSaved = BitsSaved Mod 8

        MsgBox(BytesSaved.ToString + " bytes and " + BitsSaved.ToString + " bits saved!")
...
Err:
    ...
    Private Function AddPartToDisk() As Boolean
        'On Error GoTo Err
        ...
        For I As Integer = 0 To Prgs.Count - 1
            NewLZ(Prgs(I).ToArray, FileAddrA(I),, FileLenA(I), FileIOA(I))
        Next
        CloseLastBuff()

        'MsgBox(NextTrack.ToString + ":" + NextSector.ToString)
