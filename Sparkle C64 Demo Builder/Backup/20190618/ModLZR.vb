...
NextBuffer:
        BlockUnderIO = CheckIO(PrgLen - 1)       'Check if last byte of prg could go under IO
...
FileAddress:
            ...
            'If FileUnderIO = True Then ByteCt -= 1
            Buffer(ByteCt - 1) = Int((PrgAdd + PrgLen - 1) / 256)
            ByteCt -= 2
            LitCnt = -1
        Else
            'Buffer is not empty, we need New File Match Tag
            'IIf(FileUnderIO = True, ByC = 5, ByC = 4)                       'Calculate number of bytes needed for new file
            ...
            If DataFits(ByC, BiC, LitCnt) Then                          'SeqUnderIO is NOT NEEDED here
        ...
        LastByte = ByteCt                       'The first byte of the ByteStream after and Address Bytes (253 or 252 with BlockCnt)
    ...
    Private Function LongMatch() As Boolean
        ...
        If LitCnt > -1 Then 'there is a Literal Sequence to be finished first
            ...
            Else    'Long match does not fit, check if mid match would fit
                If MaxLen > MaxFarLen Then MaxLen = MaxFarLen           'Adjust MaxLen for MidMatch
                If DataFits(2, Bits, LitCnt, CheckIO(POffset - MaxLen + 1)) = True Then
                    ...
                    GoTo BufferFull2
                Else
                    If MaxLen > MaxNearLen Then MaxLen = MaxNearLen     'Adjust MaxLen for ShortMatch
            ...
            Else
                ...
                Else
                    ...
                    If (MaxOffset <= 64) And DataFits(1, 1, LitCnt, CheckIO(POffset - MaxLen + 1)) = True Then
                        ...
                        AddNearM()
                        GoTo BufferFull2
                        'IF NEARMATCH DOES NOT FIT - LITBYTE WILL NOT FIT
                        'LITBYTE WOULD NEED AT LEAST 2 BITS And 1 Byte, NearMatch NEEDS 1 BIT And 1 BYTE
                    Else
...
BufferFull2:
    ...
    Private Function DataFits(ByteLen As Integer, BitLen As Integer, Literals As Integer, Optional SeqUnderIO As Integer = 0) As Boolean
        ...
        If FileUnderIO = True Then              'Some blocks of the file will need to go under IO
            NeededBytes += BlockUnderIO Or SeqUnderIO    'If block does or would go under IO then add 1
        End If
        ...
        If ByteCt >= BitCt + ByteLen + NeededBytes Then  '> because #$00 End Byte NEEDED
            BlockUnderIO = BlockUnderIO Or SeqUnderIO   'Data will fit so it is safe to update BlockUnderIO FLAG here
            DataFits = True
        Else
            DataFits = False
        End If
    ...
    Public Sub CloseBuffR()
        If DoDebug Then Debug.Print("CloseBuffR")
        ...
        If (FileUnderIO = True) And (BlockUnderIO = 1) Then
            'File goes under IO and this block is under IO
            For I As Integer = ByteCt + 1 To LastByte + 1   'Move all bytes, including AdHi, to left
                Buffer(I - 1) = Buffer(I)
            Next
            Buffer(LastByte + 1) = 0  'IO Flag byte
        End If
        ...
        'We have not reached the end of the file -> update buffer

        Buffer(ByteCt) = (PrgAdd + POffset) Mod 256
        'If FileUnderIO = True Then
        'If PrgAdd + POffset > &HCFFF Then ByteCt -= 1
        'End If
        ...
        If (MatchStart - POffset < LastByte - 2) And (NewFile = False) Then                     'If bytes compressed < 252/253 then
            If MatchStart >= LastByte - 3 Then
                ...
                If FileUnderIO = False Then
                    Buffer(LastByte + 2) = (PrgAdd + MatchStart - (LastByte - 2)) Mod 256  'Update AdLo and AdHi with (Forward Address+1)
                Else
                    If PrgAdd + MatchStart < &HD000 Then
                        Buffer(LastByte + 2) = (PrgAdd + MatchStart - (LastByte - 2)) Mod 256  'Update AdLo and AdHi with (Forward Address+1)
                    Else
                        Buffer(LastByte + 2) = 0
                        Buffer(LastByte + 3) = (PrgAdd + MatchStart - (LastByte - 2)) Mod 256  'Update AdLo and AdHi with (Forward Address+1)
